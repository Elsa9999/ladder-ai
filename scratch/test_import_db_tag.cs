...
<Document>
  ...
  <SW.WatchAndForceTables.PlcWatchTable ID=""0"">
    <AttributeList>
      <Name>WT_Test_Import_DB_Schema</Name>
    </AttributeList>
    <ObjectList>
      <SW.WatchAndForceTables.PlcWatchTableEntry ID=""1"" CompositionName=""Entries"">
        <AttributeList>
          <Name>""DB_PLC1_Send_To_PLC2_DB"".CmdSeq</Name>
        </AttributeList>
            ...
            string tempPath = @"D:\AI_Agent_PLC_LADDER_ONLY\scratch\test_import_db_wt.xml";
            File.WriteAllText(tempPath, xml, System.Text.Encoding.UTF8);

            var wts = plc1.WatchAndForceTableGroup.WatchTables;
            var existing = wts.Find("WT_Test_Import_DB_Schema");
            ...
            var imported = wts.Find("WT_Test_Import_DB_Schema");
            if (imported != null)
            {
