...
# We will test these versions
versions_to_test = ["3.1", "2.0", "2.1", "3.0", "4.0", "5.0"]
...
for ver in versions_to_test:
    print(f"\n--- Testing MB_CLIENT Version: {ver} ---")
    
    # Replace Version="3.1" with Version="ver"
    new_content = content.replace('Name="MB_CLIENT" Version="3.1"', f'Name="MB_CLIENT" Version="{ver}"')
    ...
    for i, line in enumerate(lines):
        if "FAILED!" in line:
            failed = True
            print(line)
            # Print the next 10 lines of context to see the exact error
            for j in range(1, 12):
                if i + j < len(lines):
                    print("  " + lines[i+j])
    if not failed:
