...
class DeleteUnusedBlocks
{
    static void Main(string[] args)
    {
        Console.OutputEncoding = System.Text.Encoding.UTF8;
        Console.WriteLine("=================================================");
        Console.WriteLine(" TIA OPENNESS UNUSED BLOCKS DELETER");
        Console.WriteLine("=================================================");
        ...
        try
        {
            ...
            foreach (var plcSoftware in softwares)
            {
                ...
                if (plcSoftware.Name.Equals("PLC_1", StringComparison.OrdinalIgnoreCase))
                {
                    DeleteBlockRecursive(plcSoftware.BlockGroup, "DB_MB_TCP_Client_Conn_DB");
                }
                else if (plcSoftware.Name.Equals("PLC_2", StringComparison.OrdinalIgnoreCase))
                {
                    DeleteBlockRecursive(plcSoftware.BlockGroup, "DB_MB_TCP_Server_Conn_DB");
                    DeleteBlockRecursive(plcSoftware.BlockGroup, "FC_MB_SERVER_COMPAT_TEST");
                }
            }
